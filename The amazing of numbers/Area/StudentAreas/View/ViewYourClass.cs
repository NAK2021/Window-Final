...
{
    public partial class ViewYourClass : Form
    {
        List<ClassStudentLearn> classList;
        StudentAreasController studentController = new StudentAreasController();
        public ViewYourClass(List<ClassStudentLearn> _classList)
        {
            InitializeComponent();
            this.classList = new List<ClassStudentLearn>(_classList);
            SubjCbox.Items.Add("Tất cả");
            _classList.ForEach(subj => SubjCbox.Items.Add(subj.subjYouLearn.course_name));
        ...
        private void ShowOnScreen(List<ClassStudentLearn> lst)
        {
            ...
            for (int i = 1; i <= numOfClass; i++)
            {
                ...
                infoOnCard.Height = 220;
                infoOnCard.BorderRadius = 6;
                //Ten lop
                string className = lst[i - 1].classYouLearn.class_id;
                //Ten mon - ma mon
                string subjName = lst[i - 1].subjYouLearn.course_name + " - " + lst[i - 1].subjYouLearn.course_id;
                //So luong: 25 sinh vien
                string numOfStus = "Giảng viên: " + lst[i - 1].lecturerYouLearn.name_;
                //Tinh trang: Da dang ky
                string status = "Tình trạng: " + lst[i - 1].status;
                //Ngay bat dau - ngay ket thuc
                DateTime dateStart = (DateTime)lst[i - 1].classYouLearn.date_start;
                DateTime dateEnd = (DateTime)lst[i - 1].classYouLearn.date_end;
                ...
                //Thu 2, Thu 4 - Ca: 1
                string dailySchedule = lst[i - 1].classYouLearn.days_ + " - " + "Ca: " + GetSectionByTime((TimeSpan)lst[i - 1].classYouLearn.section_start);
                ...
                if (status.Equals("Đang hoạt động") || status.Equals("Đã kết thúc"))
                {
        ...
        private void Filter(object sender, EventArgs e)
        {
            ...
            List<ClassStudentLearn> filterList = studentController.FilterClass("", subj, status, classList);
            SearchBar.Clear();
        ...
        private void SearchBar_TextChanged(object sender, EventArgs e)
        {
            ...
            string subj = SubjCbox.Text == "Tất cả" ? "" : SubjCbox.Text;
            string status = StatusCbx.Text == "Tất cả" ? "" : StatusCbx.Text;
            List<ClassStudentLearn> filterList = studentController.FilterClass(className, subj, status, classList);
            ShowOnScreen(filterList);
        }
    }
}
